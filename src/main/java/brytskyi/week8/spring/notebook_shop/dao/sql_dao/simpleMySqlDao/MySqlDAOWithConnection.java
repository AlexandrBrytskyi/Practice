package brytskyi.week8.spring.notebook_shop.dao.sql_dao.simpleMySqlDao;
...
public class MySqlDAOWithConnection implements IMySqlDaoWithConnection {
    ...
    * it must be added only with it`s partiya*/
    ...
    @Override
    public List<NotebookForSail> getNotebooks(NotebookState state, Date periodStateChangedStart, Date periodStateChangedEnd, Connection connection) throws SQLException {
        List<NotebookForSail> res = new LinkedList<>();
        try (PreparedStatement ps = connection.prepareStatement("SELECT " +
    ...
    public List<Prodaja> getProdajasBuyer(int buyer, Date begin, Date end, Connection connection) throws SQLException {
    ...
    @Override
    public List<Prodaja> getProdajasSeller(int seller, Date begin, Date end, Connection connection) throws SQLException {
        List<Prodaja> prodajas = new LinkedList<>();
        try (PreparedStatement ps = connection.prepareStatement("SELECT * FROM prodajas p" +
                " INNER JOIN notebooks_for_sail n ON (p.notebook=n.id) WHERE" +
